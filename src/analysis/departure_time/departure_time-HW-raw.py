...
for month_row in months[1:3]:  # replace with "months" to process all
    ...
    df_trips = (
        ...
        # same calendar day (local session TZ)
        ...
        # --- PURE RAW: use observed end of origin stop as departure time ---
        .withColumn("trip_start_epoch", col(ORIGIN_END_EPOCH_COL).cast("double"))
        .withColumn("trip_start_ts", F.to_timestamp(F.from_unixtime(col("trip_start_epoch"))))
        .withColumn("trip_start_hour", hour(col("trip_start_ts")))
    )

    w_first_hw = Window.partitionBy(UID_COL, "origin_end_date").orderBy(col("trip_start_epoch").asc())
    w_last_wh  = Window.partitionBy(UID_COL, "origin_end_date").orderBy(col("trip_start_epoch").desc())
    ...
    df_trips = (
        ...
        .withColumn("rn_hw",
            F.when(col("trip_direction") == "H_to_W", row_number().over(w_first_hw)).otherwise(F.lit(999)))
        .withColumn("rn_wh",
            F.when(col("trip_direction") == "W_to_H", row_number().over(w_last_wh)).otherwise(F.lit(999)))
    ...
    df_trips_slim = (
        df_trips
        .select(UID_COL, "trip_start_ts", "trip_start_hour")
...
ax.set_title('Percentage of Home-based Work Trips by Hour of the Day - RAW', fontsize=22, fontweight='bold', pad=22)
