...
for month_row in months[1:3]:  # replace with "months" to process all
    ...
    df_trips = (
        ...
        .filter(
            ...
            (F.col("cluster_label") != F.col("next_cluster_label")) &
            (F.col(WEEKEND_COL) == F.lit(False)) &
            (F.col("dest_weekend") == F.lit(False)) &
            (
                ((F.col(LOC_TYPE_COL) == "H") & (F.col("dest_location_type") == "W")) |
                ((F.col(LOC_TYPE_COL) == "W") & (F.col("dest_location_type") == "H"))
            ) &
            F.col(ORIGIN_END_EPOCH_COL).isNotNull() &
            F.col("dest_start_epoch").isNotNull() &
            (F.col("dest_start_epoch") > F.col(ORIGIN_END_EPOCH_COL))
        )
        .withColumn("origin_end_date", F.to_date(F.from_unixtime(F.col(ORIGIN_END_EPOCH_COL))))
        .withColumn("dest_start_date", F.to_date(F.from_unixtime(F.col("dest_start_epoch"))))
        .filter(F.col("origin_end_date") == F.col("dest_start_date"))
        # --- Uniform sample in [origin_end, dest_start] ---
        .withColumn("interval", (F.col("dest_start_epoch") - F.col(ORIGIN_END_EPOCH_COL)).cast("double"))
        .withColumn(
            "rand_start_epoch",
            F.when(F.col("interval") > 0,
                   F.col(ORIGIN_END_EPOCH_COL).cast("double") + U * F.col("interval"))
             .otherwise(F.col(ORIGIN_END_EPOCH_COL).cast("double"))
        )
        .withColumn("rand_start_ts", F.to_timestamp(F.from_unixtime(F.col("rand_start_epoch"))))
        .withColumn("trip_start_hour", F.hour(F.col("rand_start_ts")))
    )
...
ax.set_title('Percentage of Home-based Work Trips by Hour of the Day - Random Uniform', fontsize=22, fontweight='bold', pad=22)
